...
Date: 23 September 2026. Status: process addendum, with a worked planning review. These rules are not yet implemented in the Python compiler. No application behavior, domain approval or exhaustive specification coverage is certified here.
...
3. **Expand its behavioral obligations.** Record invariants, states, transitions, authority, time, races, policy snapshots and boundary failures. Instantiate only applicable register types. Every omission needs an applicability reason; a blank field is not a decision.
...
| R11–R15 | Require bounded recovery for states with a promised progression. A stable active account or indefinitely editable draft need not expire. Record overdue grace, scheduler cadence and escalation. Zero overdue rows is a drained-fixture assertion, not an unconditional production guarantee. Enforce hard expiry at the action/read boundary where required. |
...
| R20–R21 | Adopt authentication, deduplication, ordering, late/lost-event and reconciliation obligations. Include humans as dependencies where their absence can stall promised progress; not every role requires a provider-style callback contract. |
| R22 | Propagate required priority to the necessary capability slice, not its entire module or optional neighboring features. |
...
| R27–R30 | Adopt authority scope, recorded overrides and race outcomes. Define eligible overrides; structural/security invariants are not implicitly waivable. State the allowed concurrent outcomes, which may include multiple successes or an idempotent replay. |
...
| V6 | Review affected tests on semantic changes. Test corrections or harness refactoring can preserve the contract; record their rationale rather than inventing a requirement change. |
...
First extend one existing use case's authored inputs with the records above, bind its records to the existing tickets and emit the richer briefs and acceptance plan. Then implement structural checks with negative fixtures for a missing owner, uncovered late event, absent deadline mechanism, unbound real provider, and unpropagated decision. Run the old and new planners on the same scope and foundation assumptions.

Compare unresolved worker choices, missing/redundant prerequisites, criterion coverage, repair effort and whether an implementer had to invent behavior. Ticket count and fewer edges alone are not quality measures. Preserve the original trial as the comparison baseline.
